...
if ($SourceSize -lt 64) {
    throw "SourceSize must be 64 or greater."
}
...
foreach ($resolution in $Resolutions) {
    if ($resolution -lt 1) {
        throw "Resolutions must be positive integers."
    }
}
...
function New-Prompt {
    ...
    return "$($WorkflowConfig.style), $($Asset.subject). $($CategoryConfig.promptTail) Designed as an original public-safe game asset source for exact 64x64 and 128x128 exported variants. Keep the design clean, centered, high quality, no watermark."
}
...
function Invoke-SharpResize {
    ...
    $arguments += @("resize", "$Size", "$Size", "--fit", "cover", "--kernel", "nearest")
...
$workflowConfig = Get-WorkflowConfig -WorkflowName $Workflow
...
foreach ($category in $Categories) {
    ...
    foreach ($asset in $assets) {
        ...
        $sourcePrefix = "$TaskId-source-$($category)-$indexText-$($asset.name)-$($workflowConfig.slug)"
        $prompt = New-Prompt -WorkflowConfig $workflowConfig -CategoryConfig $categoryConfig -Asset $asset

        $targetPaths = @()
        foreach ($resolution in $Resolutions) {
            $resolutionSlug = "${resolution}x${resolution}"
            $fileStem = "$baseFileStem-$resolutionSlug-$($workflowConfig.slug)"
            $publicPath = Join-Path $repoRoot ("public/assets/$($categoryConfig.assetFolder)/$TaskId/$resolutionSlug/$category/$fileStem.png")
            $webpPath = Join-Path $repoRoot ("public/assets/feed-optimized/$TaskId/$resolutionSlug/$category/$fileStem-feed.webp")
        ...
        elseif ($needsSource) {
            try {
                ...
                if ($null -ne $existingSource) {
                    Write-Host "Reusing source [$Workflow][$category] $indexText $($asset.name)"
                    $sourcePath = $existingSource
                    $sourceStatus = "reused-source"
                }
                else {
                    Write-Host "Generating source [$Workflow][$category] $indexText $($asset.name)"
                    $runnerParams = @{
                        TaskId = $TaskId
                        Workflow = $Workflow
                        Prompt = $prompt
                ...
                Write-Warning "Failed source [$Workflow][$category] $indexText $($asset.name): $sourceError"
            }
        }
        ...
        foreach ($target in $targetPaths) {
            ...
            elseif ($SkipExisting.IsPresent -and (Test-Path -LiteralPath $target.publicPath)) {
                if (Test-Path -LiteralPath $target.webpPath) {
                    ...
                    try {
                        Write-Host "Optimizing existing [$($target.resolutionSlug)][$category] $indexText $($asset.name)"
                        Invoke-SharpResize -InputPath $target.publicPath -OutputPath $target.webpPath -Format "webp" -Size $target.resolution -Quality $Quality -Effort $Effort
                        ...
                        Write-Warning "Failed existing optimization [$($target.resolutionSlug)][$category] $indexText $($asset.name): $($result.error)"
                    }
                }
            }
            elseif ($sourceStatus -eq "failed") {
                ...
                try {
                    ...
                    Write-Host "Exporting [$($target.resolutionSlug)][$category] $indexText $($asset.name)"
                    Invoke-SharpResize -InputPath $sourcePath -OutputPath $target.publicPath -Format "png" -Size $target.resolution -Quality $Quality -Effort $Effort
                    Invoke-SharpResize -InputPath $target.publicPath -OutputPath $target.webpPath -Format "webp" -Size $target.resolution -Quality $Quality -Effort $Effort
                    ...
                    Write-Warning "Failed export [$($target.resolutionSlug)][$category] $indexText $($asset.name): $($result.error)"
                }
            }
    ...
    if (-not $DryRun.IsPresent) {
        foreach ($resolution in $Resolutions) {
            $resolutionSlug = "${resolution}x${resolution}"
            $packResults = @($categoryResults | Where-Object { $_.resolution -eq $resolution })
            $successful = @($packResults | Where-Object { $_.status -in @("generated", "skipped-existing", "optimized-existing") })
            $publicDir = Join-Path $repoRoot ("public/assets/$($categoryConfig.assetFolder)/$TaskId/$resolutionSlug/$category")
            $feedDir = Join-Path $repoRoot ("public/assets/feed-optimized/$TaskId/$resolutionSlug/$category")
            ...
            Write-JsonFile -Path $manifestPath -Value ([PSCustomObject]@{
                ...
                taskId = $TaskId
                packId = "$TaskId-$resolutionSlug-$category"
                workflow = $Workflow
                workflowTitle = $workflowConfig.title
                category = $category
                resolution = $resolutionSlug
            ...
            if ($successful.Count -eq $MaxItemsPerPack) {
                $media = @($successful | Sort-Object publicAsset | ForEach-Object {
                    [PSCustomObject]@{
                        type = "image"
                        url = $_.feedAsset
                        alt = "$resolutionSlug $($workflowConfig.title) pixel art $($categoryConfig.singular) asset: $($_.name -replace '-', ' ')"
                    }
                })
                $post = [PSCustomObject]@{
                    id = "$TaskId-$resolutionSlug-$category"
                    date = (Get-Date -Format "yyyy-MM-dd")
                    type = "gallery"
                    title = "$resolutionSlug Pixel Art $($categoryConfig.title) Set 01"
                    text = "Nine original public-safe $resolutionSlug pixel art $($categoryConfig.title.ToLowerInvariant()) generated through local ComfyUI and exported as exact game-ready PNG variants with lossless WebP feed previews."
                    media = $media
                    url = "https://github.com/Gameyang/GPT-GenImage2-2D-Game-Art-Resource-Test/tree/main/public/assets/$($categoryConfig.assetFolder)/$TaskId/$resolutionSlug/$category"
                    linkLabel = $categoryConfig.linkLabel
                    tags = @($categoryConfig.feedTag, $resolutionSlug, $workflowConfig.title, "Pixel Art", "Non-commercial")
                }
                ...
                Write-Warning "Pack not published because $($successful.Count)/$MaxItemsPerPack media are ready: $TaskId-$resolutionSlug-$category"
            }
        }
    }
}
...
if (-not $DryRun.IsPresent) {
    ...
    Write-JsonFile -Path $batchManifestPath -Value ([PSCustomObject]@{
        ...
        resolutions = @($Resolutions | ForEach-Object { "${_}x${_}" })
        sourceSize = "${SourceSize}x${SourceSize}"
